...
const COPY = {
  normal: {
    photo: '好，先把照片或视频发过来吧。\n可以发1-3份，发完点「✅ 完成」进入下一步。',
    name: '这位猎物，叫什么名字？\n（可以是花名/代号，不需要真名）',
    age: '年龄多少？',
    tag: '想加一句形容吗？比如气质、风格、一句话介绍。\n不想写的话，点「⏭ 跳过」。',
  },
  secret: {
    photo: '把你悄悄拍下的那张照片或视频，发过来吧。\n可以发1-3份，发完点「✅ 完成」进入下一步。',
    name: '她，叫什么名字？\n（花名/代号都行）',
    age: '年龄多少？',
    tag: '用一句话形容她给你的感觉。\n不想写的话，点「⏭ 跳过」。',
  },
...
const submissionWizard = new Scenes.WizardScene(
  ...
  // Step 1: 继续收集，直到用户点「完成」
  async (ctx, next) => {
    if (ctx.callbackQuery) {
      if (ctx.callbackQuery.data === 'finish_photos') {
        await ctx.answerCbQuery();
        if (ctx.wizard.state.media.length === 0) {
          await ctx.reply('还没收到内容呢，先发一张图片或视频吧。', photoStepKeyboard());
          return;
        }
    ...
    if (media) {
      ctx.wizard.state.media.push(media);
      if (ctx.wizard.state.media.length >= 3) {
        await ctx.reply('已经收到3份了，点「✅ 完成」继续下一步。', photoStepKeyboard());
      } else {
        await ctx.reply(
          `收到（${ctx.wizard.state.media.length}/3），继续发或点「✅ 完成」。`,
          photoStepKeyboard()
        );
      }
      return;
  ...
  async (ctx, next) => {
    const showPreview = async () => {
      const first = ctx.wizard.state.media[0];
      await sendMediaByType(ctx, first.type, first.fileId, {
...
async function handleSubmitConfirm(ctx) {
  ...
  if (!data.media || data.media.length === 0) {
    await ctx.reply('资料不完整，请重新 /start 投稿。', withMainMenu());
  ...
  console.log(
    `[投稿] user ${ctx.from.id} 提交猎物 ${submission.id} (${first.type}, priority=${isPriority})`
  );

  await ctx.editMessageCaption('收到，已经放进待发布池了。\n下一次整点通告见，记得留意反应哦。');
  await ctx.reply('还想做点别的？', withMainMenu());
  return ctx.scene.leave();
}
...
async function handleSubmitRestart(ctx) {
  ...
  await ctx.reply('随时可以重新开始：', withMainMenu());
  return ctx.scene.leave();
}
